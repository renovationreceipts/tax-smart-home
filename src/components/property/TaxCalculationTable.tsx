...
export function TaxCalculationTable({
  ...
  return <div className="space-y-8">
      ...
      <Table>
        ...
        <TableBody>
          <TaxTableRow label="Projected Home Value" value={projectedValue} />
          <TaxTableRow label="Your Adjusted Cost Basis (Purchase Price + Tracked Improvements)" value={adjustedCostBasis} className="border-b border-gray-200" />
          <TaxTableRow label="Projected Capital Gains Before Exemptions" value={projectedValue - adjustedCostBasis} />
          <TaxTableRow label="Tracked Home Improvements Reduced Your Reported Gain By" value={totalProjectCosts} className="text-green-600 font-medium" />
          <TaxTableRow label="Without Tracking, Your Reported Gain Would Have Been" value={projectedValue - property.purchase_price} />
          <TaxTableRow label="Eligible Federal Capital Gains Exemption Applied (Based on filing status)" value={exemptionAmount} />
          <TaxTableRow label="Projected Federal Taxable Gain After Exemption" value={projectedTaxableGain} className="font-medium" />
          <TaxTableRow label="Using your capital gains tax rate, your Federal cap gains tax would be" value={estimatedTax} className="font-medium" />
        </TableBody>
      </Table>
